...
esp_err_t SystemStatus::systemStatus(PsychicRequest *request)
{
    ...
    root["mdns_started"] = esp32sveltekit.mdnsStarted(); // local responder initialization only
    root["mdns_http_service_api_ok"] = esp32sveltekit.mdnsHttpServiceApiOk();
    root["largest_free_internal"] = heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
